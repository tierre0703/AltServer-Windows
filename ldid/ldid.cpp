...
public:
	...
	void open(const char* path, int flags) {
		_assert(file_ == -1);
		file_ = _syscall(_open(path, flags));
	}
...
static void Commit(const std::string& path, const std::string& temp) {
	struct _stat info;
	if (_syscall(_stat(path.c_str(), &info), ENOENT) == 0) {
#ifndef __WIN32__
...
namespace ldid {
...
#ifdef __WIN32__
			struct _stat info;
			_syscall(_stat((path + name).c_str(), &info));
			if (false);
	...
	bool DiskFolder::Look(const std::string& path) const {
		return _syscall(_access(Path(path).c_str(), R_OK), ENOENT) == 0;
	}
...
int main(int argc, char* argv[]) {
	...
	_foreach(file, files) try {
		...
		struct _stat info;
		_syscall(_stat(path.c_str(), &info));
