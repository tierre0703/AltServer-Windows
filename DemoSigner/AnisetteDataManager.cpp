...
{
	wchar_t currentDirctory[MAX_PATH];
	GetCurrentDirectoryW(MAX_PATH, currentDirctory);

	fs::path appleDirectoryPath(currentDirctory);
...
bool AnisetteDataManager::ReprovisionDevice(std::function<void(void)> provisionCallback)
{
	...
	try
	{
		...
		SignManager::instance()->setReprovisionedDevice(true);
